...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* Halo: glyphs paint over a surface-coloured outline of themselves, so the
     hairline gridlines never run through the lettering. Collision detection
     handles dots, the frontier line and other labels; this handles the grid,
     which is unavoidable background and designed to be sat on. */
  svg text.lbl{font-size:10px;fill:var(--text-secondary);paint-order:stroke;
...
(function(){
  ...
  let W=980; const H=560, L=62, Rr=22, T=20, B=52;
  ...
  function draw(rows){
    ...
    for(const r of [...fr].sort((a,b)=> b.ii-a.ii)){
      ...
      const cands=[];
      const spread = full ? [0,-13,13,-26,26,-39,39,-52,52,-65,65,-78,78,-91,91,-104,104]
                          : [0,-13,13,-26,26];
      for(const dy of spread){
        cands.push([cx+11,   cy-h/2+dy]);      // to the right
        cands.push([cx-11-w, cy-h/2+dy]);      // to the left
      }
      for(const dy of [-14,14,-27,27]) cands.push([cx-w/2, cy-h/2+dy]);

      let placed=false;
      for(const [bx,by] of cands){
        // Inflate by the halo stroke. paint-order:stroke paints a 3px outline
        // around the glyphs, so the box that lands on screen is wider and taller
        // than the advance width -- testing the un-inflated box let a label clip
        // the frontier line at narrow widths.
        const box={x:bx-2,y:by-2,w:w+4,h:h+4};
        if(!clear(box)) continue;
        t.setAttribute("x",bx);
        t.setAttribute("y",by+h-2.5);          // y is the baseline
        boxes.push(box); placed=true; break;
      }
      if(!placed){ svg.removeChild(t); dropped++; }
    }
